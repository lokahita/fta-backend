...
export default function DataTable() {
  ...
  const url_list = Config.api_domain + "/contribution/username/" + contributor;
          ...
          return list.map((row, index) => {
            ...
            return (
              <StyledTableRow key={row.id}>
                ...
                <StyledTableCell>{row.data_name}</StyledTableCell>
                <StyledTableCell>{row.filename}</StyledTableCell>
                <StyledTableCell>{formatted_datetime}</StyledTableCell>
                <StyledTableCell style={{wordBreak:"break-all"}}>{row.url}</StyledTableCell>

                <StyledTableCell>
                  <IconButton aria-label="post message" component="a" href={"#/data/preview/" + row.id}>
                    <Preview />
                  </IconButton>
                  <IconButton aria-label="post message" component="a" href={Config.api_domain + "/data/download/" + row.id} >
                    <Download />
                  </IconButton>
      ...
      <Table stickyHeader sx={{ minWidth: 700 }} aria-label="customized table">
        <TableHead>
          <TableRow>
            ...
            <StyledTableCell width="200px">Geoserver WMS URL</StyledTableCell>
